...
<div>
    <h2>@lang('site.break_reports')</h2>
</div>

<ul class="breadcrumb mt-2">
    <li class="breadcrumb-item"><a href="{{ route('admin.home') }}">@lang('site.home')</a></li>
    <li class="breadcrumb-item">@lang('site.break_reports')</li>
</ul>
...
<div class="row">
    ...
    <div class="col-md-12">
        ...
        <div class="tile">
                ...
                <div class="modal-dialog modal-dialog-centered" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="exampleModalCenterTitle">Export Break History</h5>
                            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                <span aria-hidden="true">&times;</span>
                            </button>
                        </div>
                        <form action="{{ route('admin.breaks.reports.export') }}" method="get">
                            @csrf
            ...
            {{-- filter form  --}}
            <form action="{{ route('admin.breaks.reports') }}" method="GET">
                <div class="row">
                    <div class="col-md-3">
                        <div class="form-group">
                            <label> From Date </label>
                        ...
                        <table class="table table-hover table-bordered dataTable no-footer" id="logsTable" role="grid" aria-describedby="sampleTable_info" style="width: 100%;">
                            <thead>
                                <tr>
                                    ...
                                    <th>@lang('site.request_start_time')</th>
                                    <th>@lang('site.request_end_time')</th>
                                    <th>@lang('site.request_total_time')</th>
                                    {{-- <th>@lang('site.total_time')</th> --}}
                                </tr>
                            </thead>
                            <tbody>
                                ...
                                // convert total time minutes to hours:minutes:seconds
                                // $hours = floor($entry->actual_time / 60);
                                // $minutes = floor($entry->actual_time % 60);
                                // $seconds = $entry->actual_time - (int)$entry->actual_time;
                                ...
                                <tr>
                                    ...
                                    <th>{{ $entry->name }}</th>
                                    <th>{{ $entry->code }}</th>
                                    <th>{{ date("g:i A", strtotime($entry->start_time)) }}</th>
                                    <th>{{ date("g:i A", strtotime($entry->end_time)) }}</th>
                                    <th>{{ $diff->h }}h : {{ $diff->i }}m : {{ $diff->s }}s</th>
