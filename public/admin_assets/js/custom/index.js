$(function () {
    ...
    // Active Multi-select box (pillbox) without clossing option
...
// display overlay counter for break 
function counter(timestamp) {
    let totalSeconds = (+new Date() / 1000) - timestamp;
    $.confirm({
        title: "Break",
        content: formatTotalSeconds(totalSeconds),
        onContentReady: function() {
            var self = this;
            setInterval(() => {
                totalSeconds++
                self.setContent(formatTotalSeconds(totalSeconds));
            }, 1000)
        },
        theme: 'supervan',
        buttons: {
            ...
            stopBreak: {
                text: 'Stop break',
                action: function(stopBreakButton) {
                    $('#stopForm').submit();
                }
            },
        }
